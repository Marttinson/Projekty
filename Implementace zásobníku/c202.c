...
void Stack_Init( Stack *stack ) {
	if(stack == NULL)
		...
		stack->topIndex = -1;	//-1 proto, abych nemusel zvlášť řešit první prvek
...
void Stack_Pop( Stack *stack ) {
	if(!Stack_IsEmpty(stack))
		stack->array[stack->topIndex--] = 0;	//přiřadím hodnotu 0 (což je nejspíš zbytečné, ale přijde
												//mi to fajn pro přehlednost) a zmenším index o 1
}
...
void Stack_Push( Stack *stack, char data ) {
	if(Stack_IsFull(stack))
		...
		stack->array[stack->topIndex + 1] = data;	//vložím prvek na horní index +1, proto je výchozí hodnota -1
													//jinak bych musel zvlášť ošetřovat vložení 1. prvku
		stack->topIndex++;
	}
}
...
void Stack_Dispose( Stack *stack ) {
	free(stack->array);
	stack->array = NULL;
	stack->topIndex = -1;	//přiřadím výchozí hodnotu, nejspíš bych nemusel, přiřadí se při inicializaci
							//není to však náročná operace a přijde mi to takhle jasnější
}
